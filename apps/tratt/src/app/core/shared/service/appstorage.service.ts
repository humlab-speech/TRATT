...
export class AppStorageService {
  ...
  public readonly annotationChanged: Observable<AnnotationState> = this.store
    .select(fromAnnotation.selectAnnotation)
    .pipe(
      filter((a): a is AnnotationState => a !== undefined),
      shareReplay(1),
    );
