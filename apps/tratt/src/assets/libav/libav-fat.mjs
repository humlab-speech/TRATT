...
(function () {
  ...
  function r() {
    try {
      var e = new WebAssembly.Memory({ initial: 1, maximum: 1, shared: true });
  ...
  n = LibAV;
  if (!n.base) {
    n.base = import.meta.url;
    n.base = n.base.replace(/\/[^\/]*$/, '');
  }
  n.isWebAssemblySupported = t;
  n.isThreadingSupported = r;
  function A(e) {
    e = e || {};
    var _ = !e.nowasm && t();
    var a = e.yesthreads && _ && !e.nothreads && r();
    if (!_) return 'asm';
    else if (a) return 'thr';
    else return 'wasm';
  }
  n.target = A;
  n.VER = '6.0.0-nightly.29.f420ff.ffmpeg.6.1.1';
  n.CONFIG = 'fat';
  n.DBG = '';
  n.factories = {};
  var m = {};
  m.i64tof64 = function (e, _) {
    if (!_ && e >= 0) return e;
    if (_ === -1 && e < 0) return e;
    return _ * 4294967296 + e + (e < 0 ? 4294967296 : 0);
  };
  m.f64toi64 = function (e) {
    return [~~e, Math.floor(e / 4294967296)];
  };
  m.i64ToBigInt = function (e, _) {
    var a = new DataView(new ArrayBuffer(8));
    a.setInt32(0, e, true);
    a.setInt32(4, _, true);
    return a.getBigInt64(0, true);
  };
  m.bigIntToi64 = function (e) {
  ...
  m.AVIO_FLAG_READ = 1;
  m.AVIO_FLAG_WRITE = 2;
  m.AVIO_FLAG_READ_WRITE = 3;
  m.AVIO_FLAG_NONBLOCK = 8;
  m.AVIO_FLAG_DIRECT = 32768;
  m.AVFMT_FLAG_NOBUFFER = 64;
  m.AVFMT_FLAG_FLUSH_PACKETS = 512;
  m.AVSEEK_FLAG_BACKWARD = 1;
  m.AVSEEK_FLAG_BYTE = 2;
  m.AVSEEK_FLAG_ANY = 4;
  m.AVSEEK_FLAG_FRAME = 8;
  m.AVDISCARD_NONE = -16;
  m.AVDISCARD_DEFAULT = 0;
  m.AVDISCARD_NONREF = 8;
  m.AVDISCARD_BIDIR = 16;
  m.AVDISCARD_NONINTRA = 24;
  m.AVDISCARD_NONKEY = 32;
  m.AVDISCARD_ALL = 48;
  m.AV_LOG_QUIET = -8;
  m.AV_LOG_PANIC = 0;
  m.AV_LOG_FATAL = 8;
  ...
  m.AV_LOG_TRACE = 56;
  m.AV_PKT_FLAG_KEY = 1;
  m.AV_PKT_FLAG_CORRUPT = 2;
  m.AV_PKT_FLAG_DISCARD = 4;
  m.AV_PKT_FLAG_TRUSTED = 8;
  m.AV_PKT_FLAG_DISPOSABLE = 16;
  e(
    [
      'E2BIG',
      'EPERM',
  ...
  n.LibAV = function (_) {
    ...
    var t = 'fat';
    if (a === 'asm') {
      t = _.variant || n.variant || 'fat';
    }
    var r = true;
    if (r && (_.noes6 || n.noes6)) r = false;
    var o =
      _.toImport ||
      n.toImport ||
      e +
    ...
    var s = 'direct';
    if (a === 'thr') s = 'threads';
    else if (!c && !_.noworker && typeof Worker !== 'undefined') s = 'worker';
    return Promise.all([])
      .then(function () {
        if (_.factory || n.factory) return _.factory || n.factory;
        if (n.factories[o]) return n.factories[o];
        if (s === 'worker') {
        } else if (r) {
          return import(o).then(function (e) {
            n.factories[o] = e.default;
            return e.default;
          });
        } else if (c) {
          return (n.factories[o] = require(o));
        } else if (typeof importScripts !== 'undefined') {
          importScripts(o);
          return (n.factories[o] = LibAVFactory);
        } else {
      ...
      .then(function (e) {
        if (s === 'worker') {
          i = {};
          i.worker = new Worker(o, { type: r ? 'module' : 'classic' });
          i.worker.postMessage({
            config: {
              variant: _.variant || n.variant,
              wasmurl: _.wasmurl || n.wasmurl,
            },
          });
          return new Promise(function (e, _) {
            i.on = 1;
            i.handlers = {
              ...
              onwrite: [
                function (e) {
                  if (i.onwrite) i.onwrite.apply(i, e);
                },
                null,
              ],
              onblockread: [
                function (_) {
                  try {
                    var e = null;
                    if (i.onblockread) e = i.onblockread.apply(i, _);
                    if (e && e.then && e.catch) {
                      e.catch(function (e) {
                        i.ff_block_reader_dev_send(_[0], _[1], null, {
                          error: e,
                        });
                      });
                    }
                  } catch (e) {
                    i.ff_block_reader_dev_send(_[0], _[1], null, { error: e });
                  }
                },
                null,
              ],
            ...
            i.c = function () {
              ...
              for (var e = 0; e < t.length; e++) {
                if (t[e] && t[e].libavjsTransfer)
                  r.push.apply(r, t[e].libavjs_create_main_thread);
              }
              return new Promise(function (e, _) {
                var a = i.on++;
                t = [a].concat(t);
                i.handlers[a] = [e, _];
                i.worker.postMessage(t, r);
              });
            };
            function a(e) {
              var _ = e.data[0];
              var a = i.handlers[_];
              if (a) {
                if (e.data[2]) a[0](e.data[3]);
                else a[1](e.data[3]);
                if (typeof _ === 'number') delete i.handlers[_];
              }
            }
            i.worker.onmessage = a;
          ...
          return Promise.all([])
            .then(function () {
              return e({
                wasmurl: _.warmurl || n.wasmurl,
                variant: _.variant || n.variant,
              });
            })
            .then(function (e) {
              i = e;
              var _ = i.libavjs_create_main_thread();
              var o = i.PThread.pthreads[_];
              var a = 0;
              var r = 1;
              var s = {};
              var n = null;
              ...
              i.c = function () {
                ...
                return new Promise(function (e, _) {
                  ...
                  t = [a].concat(t);
                  s[a] = [e, _];
                  o.postMessage({ c: 'libavjs_run', a: t });
                });
              };
              var c = o.onmessage;
              o.onmessage = function (e) {
                if (e.data && e.data.c === 'libavjs_ret') {
                  ...
                  if (i.readerDevReady(e.data.fd)) {
                    o.postMessage({ c: 'libavjs_wait_reader', fd: e.data.fd });
                  } else {
                    var t = i.fdName(e.data.fd);
                    var r = i.ff_reader_dev_waiters[t];
                    if (!r) {
                      r = i.ff_reader_dev_waiters[t] = [];
                    }
                    r.push(function () {
                      o.postMessage({
              ...
              i.terminate = function () {
                i.PThread.unusedWorkers
                  .concat(i.PThread.runningWorkers)
                  .forEach(function (e) {
                    e.terminate();
                  });
              };
      ...
      .then(function () {
          ...
          e.forEach(function (e) {
            ...
            i[e] = function () {
              ...
              return new Promise(function (e, _) {
                try {
                  var a = r.apply(i, t);
                  if (typeof a === 'object' && a !== null && a.then)
                    a.then(e).catch(_);
                  else e(a);
                } catch (e) {
                  _(e);
        ...
        var a = [
          ...
          'AVStream_avg_frame_rate_num',
          'AVStream_avg_frame_rate_den',
          'AVStream_avg_frame_rate_num_s',
          'AVStream_avg_frame_rate_den_s',
          'AVStream_avg_frame_rate_s',
          'AVStream_r_frame_rate_num',
          'AVStream_r_frame_rate_den',
          'AVStream_r_frame_rate_num_s',
          'AVStream_r_frame_rate_den_s',
          'AVStream_r_frame_rate_s',
