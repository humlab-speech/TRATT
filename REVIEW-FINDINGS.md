...
| S8 | MED ⚠️ | repo-wide | ~598 `any`/`as any` occurrences (originally estimated 556). **Correction:** worst files are `libs/annotation/src/lib/annotation.ts` (24), `tool-configurator.component.ts` (23), `annotation.store.service.ts` (23), `transcr-editor.component.ts` (23) — not the editor components. `editors/components.ts:7` has `editor: any`. ✅ Closed for the 4 named worst files — `annotation.ts` and `transcr-editor.component.ts` now have 0 `any`, `table-configurator.component.ts` (the file the finding's "tool-configurator" name referred to) now has 0, `annotation.store.service.ts` down to 2, see commit series ending `37883d026`. The long tail of ~600 remaining `any` occurrences outside these 4 files is NOT closed, tracked as follow-up. |
